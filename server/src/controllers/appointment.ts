...
app.post('/symptoms', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const doctor_types = doctor_types_data.map((doctor_type: any) => doctor_type.type);
        ...
        const chosen_categories = chosen_categories_data.map((c: any) => doctor_types_data[c.document]._id);

        const chosen_doctors_data = await Doctor.find({ "type": { "$in": chosen_categories }}).exec();
        const chosen_doctors = chosen_doctors_data.map((c: any) => c._id);

        const appointments = await Appointment.find({ "doctor_id": { "$in": chosen_doctors } }).exec();
